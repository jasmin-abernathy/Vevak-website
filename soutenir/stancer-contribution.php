...
function contribution_headers(): void
{
    ...
    header("Content-Security-Policy: default-src 'self'; img-src 'self' data:; style-src 'self'; form-action 'self'; base-uri 'self'; frame-ancestors 'self'");
}
...
function contribution_config(): array
{
    ...
    if (!is_readable(CONTRIBUTION_SHARED_STANCER_CONFIG)) {
        throw new RuntimeException('Configuration de paiement indisponible.');
    }
    ...
    if (!preg_match('/^s(?:test|prod)_[A-Za-z0-9]+$/', $key)) {
        throw new RuntimeException('Configuration Stancer invalide.');
    }
...
function contribution_request(string $method, string $path, ?array $payload = null): array
{
    ...
    if ($status < 200 || $status >= 300 || !is_array($data)) {
        throw new RuntimeException('Stancer a refusé la demande.');
    }
    return $data;
}
...
function contribution_amount_to_cents(mixed $value): int
{
    ...
    if ($cents < 100 || $cents > 50000) {
        throw new InvalidArgumentException('Le montant doit être compris entre 1 € et 500 €.');
    }
    return $cents;
}
